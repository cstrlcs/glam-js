...
import { Vec2, Vec3, BVec2, BVec3, BVec4 } from "./";
...
export class Vec4 {
  ...
  /** All zeroes. */
	...
	/** Creates a `Vec3` vector from `Vec4` */
	truncate(): Vec3 {
	 ...
	 */
	min(rhs: Vec4): Vec4 {
		return new Vec4(Math.min(this.x, rhs.x), Math.min(this.y, rhs.y), Math.min(this.z, rhs.z), Math.min(this.w, rhs.w));
	}
	 ...
	 */
	max(rhs: Vec4): Vec4 {
		return new Vec4(Math.max(this.x, rhs.x), Math.max(this.y, rhs.y), Math.max(this.z, rhs.z), Math.max(this.w, rhs.w));
	}
	...
	cmpeq(rhs: Vec4): BVec4 {
		return new BVec4(this.x === rhs.x, this.y === rhs.y, this.z === rhs.z, this.w === rhs.w);
	}
	...
	isNegativeBitmask(): number {
		return (Number(this.x < 0)) | (Number(this.y < 0) << 1) | (Number(this.z < 0) << 2) | (Number(this.w < 0) << 3);
	}
	...
	isNaN(): boolean {
		return Number.isNaN(this.x) || Number.isNaN(this.y) || Number.isNaN(this.z) || Number.isNaN(this.w);
	}
	 ...
	 */
	isNaNMask(): BVec4 {
		return new BVec4(Number.isNaN(this.x), Number.isNaN(this.y), Number.isNaN(this.z), Number.isNaN(this.w));
	}
	...
	divEuclid(rhs: Vec4): Vec4 {
		return new Vec4(Math.floor(this.x / rhs.x), Math.floor(this.y / rhs.y), Math.floor(this.z / rhs.z), Math.floor(this.w / rhs.w));
	}
	 ...
	 */
	trunc(): Vec4 {
		return new Vec4(Math.trunc(this.x), Math.trunc(this.y), Math.trunc(this.z), Math.trunc(this.w));
	}
	...
	*/
	...
	approxEq(rhs: Vec4 | number): boolean {
		...
		if (rhs instanceof Vec4) {
			return Math.abs(this.x - rhs.x) < threshold && Math.abs(this.y - rhs.y) < threshold && Math.abs(this.z - rhs.z) < threshold && Math.abs(this.w - rhs.w) < threshold;
		}

		return Math.abs(this.x - rhs) < threshold && Math.abs(this.y - rhs) < threshold && Math.abs(this.z - rhs) < threshold && Math.abs(this.w - rhs) < threshold;
	}
